...
| `RoutingSkill`           | One entry of the tenant's skill catalogue (`key`, `label`, `description`) | Web settings page (`TenantSettings.skills`)                                     | Agent (prompt, tool enum), API `routes/internal.ts` (filter)                              |
| `baseLanguage`           | `de-CH` → `de`                                                            | —                                                                               | API `flow.ts` (`lang:` skill), agent `worker.ts` (STT language)                           |
...
| `SoundUrl` / `Sounds`    | Configurable sounds: `http(s)` URL or `/api/public/media/<id>`            | Web settings page (`TenantSettings.sounds`, `QueueConfig.holdMusicUrl`)         | API (`flow.hold` → `MediaCommand.music`, `POST /calls` → `ringback`, desk `ringtone`)     |
| `MediaCommand`           | API → media worker: `moh.start` (token, style, `music`) / `moh.stop`      | API `flow.ts` over the Postgres bus                                             | Media worker `worker.ts`                                                                  |
| `MediaAsset`             | An uploaded sound file (metadata + public `url`, never the bytes)         | API `routes/admin.ts` (`/media-assets`)                                         | Web settings page                                                                         |
...
| ---------------------- | ---------------------------------------------------------------------------------------------------- | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `call.offer`           | `callId`, `queueKey`, `reason?`, `summary?`, `expiresAt`, `requiredSkills?`, `language?`, `relaxed?` | one user                | You are being rung. `reason`/`summary` come from the AI's `escalateToHuman` call; absent in human-first. `expiresAt` is when the offer moves to the next agent. `requiredSkills` are the skill keys the offer currently requires, `language` the caller's language, `relaxed` is true once call-pinned skills were dropped (`QueueConfig.relaxAfterSec`). |
| `call.offer.cancelled` | `callId`                                                                                             | one user                | The offer is no longer yours (timed out, taken by someone else, or the call ended).                                                                                                                                                                                                                                                                       |
| `call.updated`         | `callId`, `status: CallStatus`, `heldAt?: string \| null`                                            | whole tenant            | The call changed status or hold state (`heldAt` is the ISO time the hold started, `null` when retrieved).                                                                                                                                                                                                                                                 |
...
| `sounds.holdMusic`     | `SoundUrl`, optional        | — (synthesized loop)                         | Media worker via `MediaCommand.music`; a queue's `holdMusicUrl` overrides it.                                                            |
| `sounds.ringtone`      | `SoundUrl`, optional        | — (bundled tone)                             | Web desk, from `GET /api/desk/settings`.                                                                                                 |
| `sounds.ringback`      | `SoundUrl`, optional        | — (bundled tone)                             | Embed button, from `POST /api/public/calls` → `sounds.ringback`.                                                                         |
| `skills`               | `RoutingSkill[]`, max 50    | `[]`                                         | Agent (prompt + `escalateToHuman.skills` enum), API `POST /escalate` (unknown keys dropped), web Settings. `lang:*` skills are implicit. |

`QueueConfig.relaxAfterSec` (integer 0–300, default `20`) is how long a call waits for a qualified agent before its call-pinned skills (AI tags and the `lang:` skill) are dropped; `0` never relaxes. `baseLanguage('de-CH') === 'de'` is the normalization used for `lang:` skills.
