 ...
 *
 * The POC has a softer target of 20k: exceeding it prints a warning so the
 * drift is visible long before the hard gate trips.
 *
 * Only git-tracked files count, so generated output (dist/, docs/api/,
 * coverage/) is excluded by construction. Markdown/JSON/YAML are config and
 * docs, not code — excluded on purpose; don't dodge the gate by moving logic
 * into an uncounted extension.
 */
...
if (total > POC_TARGET) {
  console.warn(`loc: WARNING — ${total} lines exceed the ${POC_TARGET} POC target.`);
}
if (total > BUDGET) {
  console.error(
