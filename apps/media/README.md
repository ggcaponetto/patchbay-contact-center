...
- `src/music.ts` renders the hold-music loop once at startup: a soft A-major-pentatonic
  arpeggio synthesized to 16-bit PCM at 48 kHz — no audio assets to ship or license.
...
| Context | How                                                                                                        |
| ------- | ---------------------------------------------------------------------------------------------------------- |
| Dev     | `npm run dev` (part of the combined command) or `npm run dev:media`. Needs `DATABASE_URL`.                 |
...
the session lifecycle with a fake transport: start/duplicate/stop, stop-during-connect,
room-closed cleanup, failure logging, shutdown. The real LiveKit path is exercised by the
