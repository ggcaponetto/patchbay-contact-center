...
| `DEV_USER_EMAIL`                            | no       | Dev only: skip Google and sign every request in as this user; a `cc_dev_user=<email>` cookie picks another (e2e actors). Ignored when `NODE_ENV=production`. |
