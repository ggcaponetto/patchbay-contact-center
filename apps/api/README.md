...
| `DEV_USER_EMAIL`                            | no       | Dev only: skip Google and sign every request in as this user; an `x-dev-user: <email>` header (the desk's "switch user" menu, one identity per browser tab; `?as=` on the websocket) or a `cc_dev_user=<email>` cookie (`POST /api/auth/dev-switch`, e2e actors) picks another. Ignored when `NODE_ENV=production`. |
...
2. `getSession(headers)` resolves the user from the Better Auth cookie (or the dev user,
   optionally picked by the `x-dev-user` header / `cc_dev_user` cookie).
