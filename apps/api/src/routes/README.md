...
| POST   | `/calls/:id/escalate`     | `{ reason, summary, ringSec? = 20 (5..120), skills?: string[], language? }` | `{ outcome: 'accepted', agentName }` or `{ outcome: 'nobody' }` (long-poll) | 400, 404        |
...
stopped). `escalate` first pins the AI's routing tags on the call: `skills` are filtered
against the tenant catalogue (`TenantSettings.skills`; unknown keys are dropped with a
warning, `lang:*` keys pass), merged with `call.required_skills` and stored, and
`language` is stored as its base tag (`de-CH` → `de`, see `baseLanguage`). Both land in
the `escalation.requested` event. It then blocks until a human accepted or nobody could; the worker needs an
...
| Method | Path                          | Auth            | Body                                                                | Response                                                                             | Errors                                                               |
| ------ | ----------------------------- | --------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | -------------------------------------------------------------------- |
| GET    | `/settings`                   | member          | –                                                                   | `{ notReadyReasons, acwSec, …, skills: RoutingSkill[], queues }`                     | 401, 403                                                             |
...
| GET    | `/calls/:id`                  | member          | –                                                                   | call + participants (each with the desk user's `name` or `null`), transcript, events | 404 `not_found`                                                      |
